...
> ==> 时间倒序，本次更新插入位置 <==
>
> > [2025-4-10]-[戴逦]-补上周-[Transformer in Transformer]-[https://proceedings.neurips.cc/paper/2021/hash/854d9fca60b4bd07f9bb215d59ef5561-Abstract.html]-暂时无上传图片的权限
> >
> > 论文简述：作者提出TNT，基于transformer来进行图像特征提取。论文的主要思想是通过将输入图像划分为局部块，并将这些块视为“视觉句子”，进一步细分为“视觉词”，从而增强特征提取能力。TNT 架构包含两个变换器模块：一个内部变换器模块，用于建模视觉词之间的关系（即局部特征）；另一个外部变换器模块，用于建模视觉句子之间的关系（即全局特征）。但这种模型的性能仍然不如EfficientNet模型
> 
